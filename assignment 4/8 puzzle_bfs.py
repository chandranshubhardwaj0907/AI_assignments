...
def solve_eight_puzzle(initial_state, goal_state):
    heap = []
    visited = set()
    heapq.heappush(heap, (misplaced_tiles(initial_state, goal_state), initial_state))
    ...
    while heap:
        ...
        for action, new_pos in possible_actions(current_state):
            ...
            if tuple(map(tuple, new_state)) not in visited:
                heapq.heappush(heap, (misplaced_tiles(new_state, goal_state), new_state))
...
if result:
    ...
    print("Sequence of actions is as follows:")
    print(actions)
    print(goal_state)
else:
